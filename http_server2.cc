...
int main(int argc, char * argv[]) {
    ...
    int connectionsMax = CONNECTIONS_INIT;
    socklen_t addrlen;
    fd_set set;
    char buffer[BUFSIZE];
    ...
    connections = (int*) malloc(sizeof(int) * CONNECTIONS_INIT);
    ...
    while (1) {
        ...
        int ready = minet_select(socketMax, &set, NULL, NULL, &timeout);
        ...
        for (i = 0; i < connectionsCount; i++) {
	        ...
	        fprintf(stderr, "Entered handling conditions for socket %d\r\n", connections[i]);        
	            rc = handle_connection(connections[i], buffer);
...
void addConnection(int connect, int** connections, int* count, int* max) {
    if (*count == *max) {
        *max = 2 * (*max);
        *connections = (int*)realloc(*connections, (*max) * sizeof(int));
    }
